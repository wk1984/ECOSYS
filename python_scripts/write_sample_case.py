...
if __name__ == '__main__':
    ...
    my_soil_config = {
        "SOIL_GLOBALS": {
            ...
            'litter_type_manure': 0.0, 'num_surface_layers': 1.0, 'num_max_rooting_layers': 8.0,
            'num_additional_layers_w_data': 0.0, 'num_additional_layers_wo_data': 0.0, 'profile_type': 1.0
        },
        "SOIL_LAYERS": {
            'layer_depth_bottom_m': [0.01, 0.05, 0.15, 0.3, 0.5, 0.7, 1.16, 1.52] + [0.0]*11,
            'bulk_density_mg_m3': [1.1, 1.1, 1.21, 1.21, 1.47, 1.47, 1.47, 1.47] + [0.0]*11,
            'field_capacity_m3_m3': [-1.0]*19, 'wilting_point_m3_m3': [-1.0]*19,
            'vertical_ksat_mm_h': [-1.0]*19, 'lateral_ksat_mm_h': [-1.0]*19,
            'sand_contents_kg_mg': [319.71]*4 + [330.14]*4 + [180.0]*3 + [0.0]*8,
            'silt_contents_kg_mg': [545.39]*4 + [184.49]*4 + [20.0]*3 + [0.0]*8,
            'macropore': [0.0]*19, 'rock_fraction': [0.0]*19,
            'ph': [5.77]*4 + [5.8]*4 + [4.35]*7 + [0.0]*4,
            'cation_exchange_capacity': [10.0]*8 + [5.0]*3 + [0.0]*8,
            'anion_exchange_capacity': [3.0]*11 + [0.0]*8,
            'total_soc_kg_mg': [3.87, 2.87, 2.6, 2.6, 2.0, 2.0, 2.0, 2.0, 3.0, 0.5, 0.5] + [0.0]*8,
            'poc_kg_mg': [0.0]*19, 'son_g_mg': [-1.0]*19, 'sop_g_mg': [-1.0]*19,
            'soluble_exch_nh4_g_mg': [3.0]*3 + [1.0]*3 + [0.0]*13,
            'soluble_exch_no3_g_mg': [12.0]*3 + [1.0]*3 + [0.0]*13,
            'soluble_exch_h2po4_g_mg': [10.0]*12 + [0.0]*7,
            'soluble_al_g_mg': [-1.0]*11 + [0.0]*8, 'soluble_fe_g_mg': [-1.0]*11 + [0.0]*8,
            'soluble_ca_g_mg': [40.0]*19, 'soluble_mg_g_mg': [0.0]*3 + [18.0]*2 + [0.0]*14,
            'soluble_na_g_mg': [0.07]*3 + [0.05]*2 + [0.01]*6 + [0.0]*8,
            'soluble_k_g_mg': [0.0]*19, 'soluble_so4s_g_mg': [48.0]*11 + [0.0]*8,
            'soluble_cl_g_mg': [35.0]*11 + [0.0]*8, 'alpo4_mineral_g_mg': [50.0]*11 + [0.0]*8,
            'fepo4_mineral_g_mg': [50.0]*11 + [0.0]*8, 'cahpo4_mineral_g_mg': [0.0]*19,
            'apatite_mineral_g_mg': [0.0]*19, 'aloh3_mineral_g_mg': [1000.0]*11 + [0.0]*8,
            'feoh3_mineral_g_mg': [1000.0]*11 + [0.0]*8, 'caso4_mineral_g_mg': [0.0]*19,
            'caco3_mineral_g_mg': [0.0]*19, 'gapon_ca_nh4': [1.0]*19,
            'gapon_ca_h': [0.25]*11 + [0.0]*8, 'gapon_ca_al': [0.25]*11 + [0.0]*8,
            'gapon_ca_mg': [0.6]*11 + [0.0]*8, 'gapon_ca_na': [0.16]*11 + [0.0]*8,
            'gapon_ca_k': [3.0]*11 + [0.0]*8,
            'initial_water_contents': [1.0]*11 + [0.0]*8, 'initial_ice_contents': [0.0]*19,
            'initial_c_fine_litter': [45.0, 60.0, 75.0, 75.0, 60.0, 60.0, 45.0, 45.0, 15.0] + [0.0]*10,
            'initial_n_fine_litter': [1.5, 2.0, 2.5, 2.5, 2.0, 2.0, 1.5, 1.5, 0.5] + [0.0]*10,
            'initial_p_fine_litter': [0.15, 0.2, 0.25, 0.25, 0.2, 0.2, 0.15, 0.15, 0.05] + [0.0]*10,
            'initial_c_woody_litter': [0.0]*19, 'initial_n_woody_litter': [0.0]*19,
            'initial_p_woody_litter': [0.0]*19, 'initial_c_manure_litter': [0.0]*19,
            'initial_n_manure_litter': [0.0]*19, 'initial_p_manure_litter': [0.0]*19
        }
    }
    ...
    # 2.1 生成 Runscript
    io_handler.write_runscript_from_config(my_config, "runscript_generated.txt")
    
    # 2.2 生成 Site, Topography, and Soil data
    io_handler.write_sitedata_from_config(my_site_config, "st022852.txt")
    io_handler.write_topography_from_config(my_topo_config, "tp022852.txt")
    io_handler.write_soildata_from_config(my_soil_config, "s022852.txt")

    # 2.3 生成植被参数文件
    io_handler.write_crop_params_from_config(grass62_config, "gras62")

    # 2.4 生成植被管理文件
    io_handler.write_plant_management_file(pft_arctic_p_config, "pft_arctic_p")
    io_handler.write_plant_management_file(pft_arctic_g_config, "pft_arctic_g")
    ...
    for year in range(1800, 1806):
        ...
        io_handler.write_weather_options_from_config(opt_config, f"opt{year}")
