...
C
     ...
     2.OR.WTSTK(NZ,NY,NX).LE.ZEROP(NZ,NY,NX))THEN
      ...
      ELSE
      FWODB(1)=1.0
      FWOOD(1)=AMIN1(SQRT(WVSTK(NZ,NY,NX)/WTSTK(NZ,NY,NX)),1.0)
      FWODR(1)=AMIN1(SQRT(FRTX*WVSTK(NZ,NY,NX)/WTSTK(NZ,NY,NX)),1.0)
      ENDIF
      FWODB(0)=AMAX1(1.0-FWODB(1),0.0)
      FWOOD(0)=AMAX1(1.0-FWOOD(1),0.0)
      FWODR(0)=AMAX1(1.0-FWODR(1),0.0)
...
8822  FORMAT(A8,7I4,12E12.4)
      ...
      FWODLN(0)=AMIN1(FWODB(0)*CNSTK(NZ,NY,NX)/CNLFW,1.0)
      FWODLP(0)=AMIN1(FWODB(0)*CPSTK(NZ,NY,NX)/CPLFW,1.0)
      FWODSN(0)=AMIN1(FWODB(0)*CNSTK(NZ,NY,NX)/CNSHW,1.0)
      FWODSP(0)=AMIN1(FWODB(0)*CPSTK(NZ,NY,NX)/CPSHW,1.0)

      FWOODN(0)=AMIN1(FWOOD(0)*CNSTK(NZ,NY,NX)/CNRTW,1.0)
      FWOODP(0)=AMIN1(FWOOD(0)*CPSTK(NZ,NY,NX)/CPRTW,1.0)
      FWODRN(0)=AMIN1(FWODR(0)*CNRT(NZ,NY,NX)/CNRTW,1.0)
      FWODRP(0)=AMIN1(FWODR(0)*CPRT(NZ,NY,NX)/CPRTW,1.0)

      FWODLN(1)=AMAX1(1.0-FWODLN(0),0.0)
      FWODLP(1)=AMAX1(1.0-FWODLP(0),0.0)
      FWODSN(1)=AMAX1(1.0-FWODSN(0),0.0)
      FWODSP(1)=AMAX1(1.0-FWODSP(0),0.0)
      FWOODN(1)=AMAX1(1.0-FWOODN(0),0.0)
      FWOODP(1)=AMAX1(1.0-FWOODP(0),0.0)
      FWODRN(1)=AMAX1(1.0-FWODRN(0),0.0)
      FWODRP(1)=AMAX1(1.0-FWODRP(0),0.0)
C      write(556,*)I*100+J,FRTX,FRTX*WVSTK(NZ,NY,NX),WVSTK(NZ,NY,NX)
C     2,WTSTK(NZ,NY,NX)
C      write(556,*)CNLFW,CPLFW,CNSHW,CPSHW,CNRTW,CPRTW
C      write(556,*)FWODB,'shoot',FWODLN,FWODLP,'leaf',FWODSN
C     2,FWODSP,'pet'
C      write(556,*)FWOOD,'wood',FWOODN,FWOODP
C      write(556,*)FWODR,'root',FWODRN,FWODRP
C      write(556,*)'------------------'
C
...
C
     ...
     2+WTEABN(NB,NZ,NY,NX)+WTGRBN(NB,NZ,NY,NX))
      ENDIF
C      write(556,*)part,'part'
C      write(556,*)DMSHT,DMSHD,'shoot-root'
C      write(556,*)CNSHX,CPSHX,'shootx'      
...
C
      ...
      write(*,*)I*100+J,'computegpp',NB,NZ
      write(456,*)'compgpp'
      write(456,*)FDBK(NB,NZ,NY,NX),SSIN(NY,NX)
     2,RADP(NZ,NY,NX),CO2Q(NZ,NY,NX)
      endif
...
C
      if(I==6.and.J==22.and..false.)then
      write(456,*)'c3',K,VCGRO(K,NB,NZ,NY,NX)
      endif
      DO 210 L=JC,1,-1
      IF(ARLFL(L,K,NB,NZ,NY,NX).GT.ZEROP(NZ,NY,NX))THEN
C
...
C
      if(I==6.and.J==22.and..false.)then
      CO2Q(NZ,NY,NX)=350.87925937554496
      PAR(N,M,L,NZ,NY,NX)=541.09613779244762
      CO2I(NZ,NY,NX)=245.61547738007474
      endif
      DO 215 N=1,4
      DO 220 M=1,4
C
...
C     ENDIF
      ...
      ENDIF
      if(I==6.and.J==22.and..false.)then
      write(456,*)'N,M',N,M,'pardir',PAR(N,M,L,NZ,NY,NX)
      write(456,*)'cangas',CO2Q(NZ,NY,NX),CO2E(NY,NX),CO2I(NZ,NY,NX)
      write(456,*)VL,SURFX(N,L,K,NB,NZ,NY,NX)
     2,TAUS(L+1,NY,NX)
      write(456,*)'sco2',FMOL(NZ,NY,NX),SCO2(NZ,NY,NX),XKCO2O(NZ,NY,NX)
      stop
      endif            
C
C     CO2 FIXATION IN MESOPHYLL BY SHADED LEAVES
...
C      write(556,*)'stalk',WTSTKB(NB,NZ,NY,NX),GROSTK
...
C
C      write(556,*)ETOL,SNL1(NZ,NY,NX),(WTSTKB(NB,NZ,NY,NX)/PP(NZ,NY,NX))
C      write(556,*)'StalkLenGrowth',GROH,GRO,PP(NZ,NY,NX),SNL
C     2,WTSTKB(NB,NZ,NY,NX)/PP(NZ,NY,NX),SNL2
...
C      write(556,*)'area',ASTV,VSTK,HTNODE(K1,NB,NZ,NY,NX),RSTK,'sap0'
C      write(556,*)'sap',WVSTKB(NB,NZ,NY,NX),WTSTKB(NB,NZ,NY,NX)
C     IF(NZ.EQ.1)THEN
